...
6. Discord gateway와 Direct Worker를 별도 LaunchAgent/systemd service로 구성합니다. bot 재시작이 agent 하위 프로세스를 죽이지 않아야 합니다.
...
4. 접속 방법을 확인합니다. 로컬 머신이면 현재 shell을 사용하고, 원격 서버면 기존 SSH host alias 또는 `user@host`를 받습니다. VPN, bastion, 특정 SSH key가 필요한지도 확인합니다.
...
9. Discord API로 그 컴퓨터의 category와 Codex parent 채널, 선택적인 Claude Code parent 채널을 생성하고 권한을 설정합니다. 기존 컴퓨터가 사용하는 channel ID를 절대 재사용하지 않습니다.

...
1. 저장소의 현재 branch, commit, dirty worktree, 실행 중인 bot/worker와 활성 job을 먼저 확인합니다.
2. 이 문서만 믿고 덮어쓰지 말고 실제 코드, `package.json`, `.env.example`, 기존 `.connect/config.json`, 서비스 정의를 대조합니다.
3. 기본 배포는 외부 API를 열지 않는 **Direct mode**를 선택합니다.
4. Codex runner는 `CODEX_DISCORD_CODEX_RUNNER=app-server`로 설정합니다. `exec`는 오래된 CLI 호환이 꼭 필요할 때만 사용합니다.
5. Discord gateway와 Direct Worker를 별도 LaunchAgent/systemd service로 구성합니다. bot 재시작이 agent 하위 프로세스를 죽이지 않아야 합니다.
6. 기존 사용자 수정, secret, 실행 중 작업을 보존합니다. `.env`, `.connect/`, token, transcript를 commit하지 않습니다.
7. 사용자에게는 Discord에서 직접 준비해야 하는 값만 물어봅니다. token을 답변, 로그, commit message에 다시 출력하지 않습니다.
8. 설치 후 `pnpm typecheck`, `pnpm test`, 서비스 상태, Discord ready 로그와 짧은 왕복 요청을 검증합니다.
...
| `apps/discord-bot/src/responses.ts` | Discord payload, embed, 답변 분할, 첨부 출력 형식 |
...
| `apps/local-agent/src/claudeRunner.ts` | Claude Code headless stream-json, resume/fork, 권한 모드 |
...
- `.connect/state.json`: channel/session mapping, sync, schedule, notification 상태
...
Direct worker service
  ...
  - SIGTERM 시 새 job을 받지 않고 active job drain
```
...
REPO_ROOT="/absolute/path/to/Codex_discord"
...
이 절은 새 Mac/Ubuntu 인스턴스를 실제 Discord에 연결하기 전에 반드시 확인하는 운영 기준입니다.
...
- Ubuntu system service의 `User=`와 Mac LaunchAgent의 login user를 명시적으로 확인합니다.
...
- Codex parent와 Claude parent는 서로 다른 ID여야 합니다.
...
- 정식 LaunchAgent/systemd service 외에 임시 refresh service를 만들지 않습니다.
...
| `/howtouse` | 현재 agent 세션에 사용자의 일반 Discord 첨부 입력과 agent 결과 파일 전송 형식을 전달합니다. |
...
2026-07-22 현재 개발 및 실제 Mac 서비스에서 확인한 기준은 다음과 같습니다.
...
| `/howtouse` | 현재 Codex 세션에 사용자의 일반 Discord 첨부 입력과 agent 결과 파일 전송 형식을 안내합니다. |

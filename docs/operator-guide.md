...
Agent Relay publishes the currently active participant to every Connector through the trusted private control channel. In an active Codex relay thread, an authorized ordinary user message retains the same implicit-steering behavior. In the waiting peer thread, an agent request is suppressed and the Connector links the active thread instead. Active headless Claude Code cannot be live-steered, so the Connector asks the user to wait or stop the relay. `/agent-chat-stop` sends an exact request-ID cancellation marker: a queued relay job is removed, while a matching running Codex or Claude Code job receives an interrupt. Late cancellation markers cannot interrupt a newer unrelated turn.

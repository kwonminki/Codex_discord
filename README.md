...
| `/fork` | Claude Code session thread에서 이름 입력 모달을 열고, 현재 Claude session을 새 Discord thread로 fork합니다. |
...
동기화된 세션 채널에서 Discord로 보낸 요청은 항상 같은 Codex 세션에 `resume`되므로, Codex Desktop 쪽에서도 같은 세션 안에서 새 사용자 요청과 처리 과정이 이어집니다. Claude Code thread에서는 `/fork`를 실행해 새 이름을 입력하면 기존 Claude session을 `--fork-session`으로 분기하고, 같은 부모 채널 아래에 새 Discord thread를 만듭니다.

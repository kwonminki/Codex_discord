...
`CODEX_DISCORD_CODEX_SANDBOX` 값은 `read-only`, `workspace-write`, `danger-full-access` 중 하나입니다. `danger-full-access`는 Codex의 OS sandbox를 풀기 때문에 신뢰하는 private Discord 서버와 신뢰하는 머신에서만 사용하세요. `codex exec` runner에서는 `CODEX_DISCORD_CODEX_BYPASS_APPROVALS_AND_SANDBOX=1`도 지원하지만, app-server runner를 쓰는 일반 Mac 설정에서는 `danger-full-access`를 권장합니다.

...
별도의 Coordinator Bot을 활성화한 서버에서는 `/agent-chat`으로 현재 스레드와 다른 agent 스레드를 연결할 수 있습니다. 두 agent의 최종 공개 답변과 Discord 첨부파일을 번갈아 전달하며, 양쪽이 종료에 동의하거나 설정한 라운드·시간 제한에 도달하면 Operator 역할을 한 번 멘션합니다. `/agent-chat-status`로 상태를 확인하고 `/agent-chat-stop`으로 다음 relay를 중단할 수 있습니다.

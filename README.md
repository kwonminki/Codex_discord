...
Codex 작업이 실행 중일 때 같은 Discord thread에 보내는 일반 메시지는 활성 app-server turn에 즉시 steering됩니다. 현재 작업을 건드리지 않고 다음 turn으로 남기려면 `/queue prompt:<요청>`을 사용합니다. 활성 turn이 없거나 steering 요청이 경쟁 상태로 늦게 도착하면 메시지를 잃지 않고 FIFO 대기열의 다음 요청으로 처리합니다. `/steer`는 명시적 steering 별칭으로 계속 지원하고, prompt 없는 `/queue`는 현재 실행과 대기 요청을 보여줍니다. 후속 agent 요청이 남아 있으면 중간 turn의 완료 멘션은 생략하고 큐가 모두 끝났을 때 한 번만 알립니다. 실패와 권한 요청은 즉시 멘션합니다. Claude Code headless 실행은 live steering을 지원하지 않으므로 작업 중 일반 메시지와 `/queue prompt:<요청>`이 모두 다음 turn으로 대기합니다. 대기열은 봇 프로세스 메모리에만 있고 재시작하면 유지되지 않습니다.

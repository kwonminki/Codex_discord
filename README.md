...
Mac, Windows, Ubuntu 서버에서 실행되는 **Codex와 Claude Code를 Discord 스레드로 사용하는 개인용 브리지**입니다.
...
에이전트는 대화 언어와 운영체제를 알아서 확인하고, 필요한 Discord 설정과 로컬 서비스를 구성합니다. 첫 컴퓨터가 준비되면 추가로 연결할 Mac, Windows 또는 Ubuntu 서버가 있는지도 물어봅니다.

...
같은 Discord 채널의 일반 메시지는 FIFO 대기열에서 순서대로 실행됩니다. `/steer`, `/interrupt`, `/queue`, `/queue-clear`와 권한 승인 응답은 대기열을 우회합니다. Codex의 `/steer`와 `/interrupt`는 app-server 모드의 활성 turn에 직접 전달되며, Claude Code headless 실행에서는 live turn 제어를 지원하지 않으므로 안내 메시지만 표시됩니다. 대기열은 봇 프로세스 메모리에만 있고 재시작하면 유지되지 않습니다.

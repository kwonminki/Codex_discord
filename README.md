...
| `/steer prompt:<지시>` | 현재 실행 중인 Codex turn에 새 지시를 즉시 추가합니다. Claude Code에서는 지원 안내를 표시합니다. |
| `/interrupt` | 현재 실행 중인 Codex turn에 중단 요청을 보냅니다. Claude Code에서는 지원 안내를 표시합니다. |
| `/queue` | 현재 채널의 실행 중 작업과 대기 요청을 보여줍니다. |
...
Discord에서 직접 요청한 Codex 및 Claude Code의 최종 답변이 한 메시지 제한을 넘으면, 봇은 문단과 줄바꿈을 우선해서 여러 메시지로 나눠 순서대로 전송합니다. 코드 블록은 각 분할 메시지 안에서 닫고 다음 메시지에서 다시 열며, 모든 답변 조각을 보낸 뒤에만 operator role이 포함된 완료 알림을 보냅니다.
...
동기화된 세션 채널에서 Discord로 보낸 요청은 항상 같은 Codex 세션에 `resume`되므로, Codex Desktop 쪽에서도 같은 세션 안에서 새 사용자 요청과 처리 과정이 이어집니다. Codex/Claude Code session thread에서는 `/fork`를 실행해 새 이름을 입력하면 기존 agent session을 분기하고, 같은 부모 채널 아래에 새 Discord thread를 만듭니다. Codex fork는 `CODEX_DISCORD_CODEX_RUNNER=app-server` 모드에서 Codex app-server의 `thread/fork`를 사용합니다.

같은 Discord 채널의 일반 메시지는 FIFO 대기열에서 순서대로 실행됩니다. 작업 중 평범한 후속 메시지를 보내면 현재 turn을 끊거나 steering하지 않고 다음 요청으로 대기하며, 앞 요청이 끝난 뒤 같은 세션에서 처리됩니다. 후속 agent 요청이 남아 있으면 중간 turn의 완료 멘션은 생략하고 큐가 모두 끝났을 때 한 번만 알립니다. 실패와 권한 요청은 즉시 멘션합니다. `/steer`, `/interrupt`, `/queue`, `/queue-clear`와 권한 승인 응답은 대기열을 우회합니다. Codex의 `/steer`와 `/interrupt`는 app-server 모드의 활성 turn에 직접 전달되며, Claude Code headless 실행에서는 live turn 제어를 지원하지 않으므로 안내 메시지만 표시됩니다. 대기열은 봇 프로세스 메모리에만 있고 재시작하면 유지되지 않습니다.
...
Codex와 Claude Code thread에서는 agent가 직접 작성한 중간 설명만 별도 메시지로 전송하되 role을 mention하지 않습니다. 명령 실행, 파일 수정, 검색, `item.started`, `생각 중`, `답변 작성 중` 같은 상태 이벤트는 진행 피드에 게시하지 않습니다. 마지막 agent 메시지는 최종 답변으로 간주해 중간 피드에서 제외하며, 동일 문구를 제거하고 작업당 최대 40개까지 보냅니다. 즉시 확인이 필요한 권한 요청과 최종 완료·실패 메시지에만 operator role을 mention합니다. 새 요청 직전 transcript 동기화는 기준점만 갱신하고 이전 대화를 다시 게시하지 않습니다.

## Codex 권한 요청 처리

Codex app-server runner는 Discord에서 실행하는 작업을 로컬/서버 자동화 용도로 쓰기 위해 기본적으로 아래 권한 설정으로 실행됩니다.
